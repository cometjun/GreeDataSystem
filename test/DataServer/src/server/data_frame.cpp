...
void data_frame::Construct96Response(data_frame& data_,uint8_t result) {
	memset(header_,0,HEADER_LENGTH);
	if (data_.cmd() != 0x96) {
...
void data_frame::ConstructF4Response(data_frame& data_,uint8_t result) {
	memset(header_,0,HEADER_LENGTH);
	if (data_.cmd() != 0xF4 ) {
		DEBUG(server::ByteToHex(data_.header_, data_.length()).c_str());
		return;
...
std::string data_frame::sys_identity() {
	if (this->cmd() == 0x90) {
		...
		return "89" + server::ByteToHex(&this->header_[21],2);
	}
	return "unknown";
}
