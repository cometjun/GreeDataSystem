...
std::string data_frame::src_address() {
	return UTIL::ByteToHex(&header_[9], 7);
}
...
std::string data_frame::dst_address() {
	return UTIL::ByteToHex(&header_[2], 7);
}
...
void data_frame::Construct90Response(data_frame& data_) {
	memset(header_, 0, HEADER_LENGTH);
	if (data_.cmd() != 0x90) {
		DEBUG(UTIL::ByteToHex(data_.header_, data_.length()).c_str() );
		return;
	}
...
void data_frame::Construct93Response(std::string srcAddress, uint8_t result) {
	...
	this->header_[0] = 0x7e;
	this->header_[1] = 0x7e;
	srcAddress = UTIL::HexStringToByteString(srcAddress);
	if (!srcAddress.empty())
...
void data_frame::Construct92Response(std::string srcAddress, uint8_t result) {
	memset(header_, 0, HEADER_LENGTH);
	this->header_[0] = 0x7e;
	this->header_[1] = 0x7e;
	srcAddress = UTIL::HexStringToByteString(srcAddress);
	...
	this->header_[16] = 0x00;
	this->header_[17] = 0x02;	// data length
	this->header_[18] = 0x92;	// cmd
	this->header_[19] = result;
	this->header_[20] = this->crc((uint8_t*) &this->header_[2], 20 - 2);
}


void data_frame::Construct92Frame(std::string srcAddress, std::string dstAddress,uint8_t result) {
	memset(header_, 0, HEADER_LENGTH);
	this->header_[0] = 0x7e;
	this->header_[1] = 0x7e;

	srcAddress = UTIL::HexStringToByteString(srcAddress);
	dstAddress = UTIL::HexStringToByteString(dstAddress);

	if (!dstAddress.empty())
		memcpy(&this->header_[2],dstAddress.c_str(),7); 	// 目的地址
	if (!srcAddress.empty())
		memcpy(&this->header_[9], srcAddress.c_str(), 7); // 源地址
...
void data_frame::ConstructF3Response(data_frame& data_) {
	...
	this->header_[19] = (now.date().year() - 1900) & 0xFF;
	this->header_[20] = (now.date().month() - 1) & 0xFF;
...
void data_frame::ConstructHeartBeat(std::string dstAddress) {
	memset(header_, 0, HEADER_LENGTH);
	...
	if (!dstAddress.empty())
		memcpy(&this->header_[2], dstAddress.c_str(), 7); //  dst address
...
std::string data_frame::sys_identity() {
	if (this->cmd() == 0x90) {
		...
		return "89" + UTIL::ByteToHex(&this->header_[19], 2);
	}
	return "unknown";
}

uint16_t data_frame::GetCheckCode(const char * mac) {
	return mac[0] * 3 + mac[1] * 6 + mac[2] * 0 + mac[3] * 9 + mac[4] * 7
			+ mac[5] * 4 + mac[6] * 10;
}
